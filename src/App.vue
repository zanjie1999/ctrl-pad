    ...
    <div class="background" :style="{
      ...
      backgroundColor: state.bgMutedColor,
        ...
        <div :class="[state.useLightMode ? 'light-mode' : 'dark-mode']">
            ...
            <var-swipe ref="swipeRef" class="swipe" @change="pageChange">
                <!-- 凌晨大时间显示器 -->
                ...
                <var-swipe-item>
                    ...
                    <div class="main-box-top">
                        <rss ref="rssRef" :openWeb="openWeb" class="shadow" />
                    </div>
                    <!-- 主页左 -->
                    <div class="main-box">
                    ...
                    <div class="main-box main-box-right">
                        ...
                        <todoList ref="todoListRef" class="shadow" />
                    </div>
                </var-swipe-item>
                <!-- 浏览器 -->
                <var-swipe-item>
                    <var-app-bar title-position="center">
                        <template #left>
                            <var-button round text color="transparent" text-color="#fff" @click="backButton()">
                                <var-icon name="chevron-left" :size="35" />
                            </var-button>
                            <var-input :hint="false" :line="false" :placeholder="state.iframeSrc" v-model="state.iframeSrcInp" @keyup.enter="iframeSrcInpEnter" style="width: 75vw; overflow: hidden;" />
                        </template>

                        <template #right>
                            <var-menu :offset-y="38" :offset-x="-20" v-model:show="state.isWebListMenuOpen">
                                <var-button round text color="transparent" text-color="#ffffff" @click="state.isWebListMenuOpen = !state.isWebListMenuOpen">
                                    <var-icon name="menu" :size="35" />
                                </var-button>

                                <template #menu>
                                    <div class="menu-list">
                                        <var-cell class="menu-cell" v-for="v in state.webList" :key="v.n" @click="openWeb(v.u)">
                                            {{ v.n }}
                                        </var-cell>
                                    </div>
                                </template>
                            </var-menu>
                            <var-button round text color="transparent" text-color="#fff" @click="homeButton()">
...
const openWeb = (url) => {
    ...
    if (url) {
        swipeRef.value.to(state.isDawn ? 2 : 1);
        state.iframeSrc = url;
    }
};
...
const uiharu = () => {
    // console.log('bar',rssRef.value.update)
    // rssRef.value.update();
    location.reload();
}
</script>
